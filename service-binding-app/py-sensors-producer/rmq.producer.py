...
class ExamplePublisher(object):
    ...
    def publish_message(self):
        ...
        msg = {"id":"{}".format(self._uuid),
               "temperature":random.uniform(0.00,100.0),
...
def main():
    ...
    #os.environ["SERVICE_BINDING_ROOT"] = "/Users/jefwang/work/bindings"
    try:
    ...
    bindings_list = sb.bindings("rabbitmq")
    if len(bindings_list) < 1:
        exit(1)

    r_host = bindings_list[0]["host"]
    r_password = bindings_list[0]["password"]
    r_port = bindings_list[0]["port"]
    r_provider = bindings_list[0]["provider"]
    r_type = bindings_list[0]["type"]
    r_username = bindings_list[0]["username"]

    print(r_host, r_password,r_port)
    conn_str = "amqp://{}:{}@{}:{}/%2F?connection_attempts=3&heartbeat=3600".format(r_username,r_password,r_host,r_port)
    print(conn_str)

    # Connect to localhost:5672 as guest with the password guest and virtual host "/" (%2F)
    example = ExamplePublisher(conn_str)
